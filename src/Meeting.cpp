...
void Meeting::removeParticipator(const std::string &t_participator) {
	if (isParticipator(t_participator)) {
		std::vector<string>::iterator it = find(m_participators.begin(), m_participators.end(), t_participator);
...
bool Meeting::isParticipator(const std::string &t_username) const {
	auto it = find(m_participators.begin(), m_participators.end(), t_username);
